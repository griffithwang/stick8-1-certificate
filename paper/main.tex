...
\author{\textit{Author information pending}}
...
small perturbations of its vertices. Indeed, nonincident compact edges
have positive distance, and incident edges remain distinct near their
common vertex. A sufficiently small linear vertex deformation is a
polygonal isotopy. The usual isotopy-extension theorem for locally flat
polygonal knots turns it into an ambient isotopy. This use of elementary
PL topology is external to Lean.
...
invertible on its image. For a compact polygon in this half-space these
maps give an isotopy from the identity to $F$ followed by reflection in
the last coordinate. Isotopy extension therefore justifies the phrase
``up to mirror image.'' We do not infer ambient knot equivalence merely
from the existence of an arbitrary diffeomorphism of an open set.
...
This is the elementary disk-supported polygonal isotopy. General
...
The accompanying release is intended to contain the source dependency
closure, pinned build instructions, finite data, and a hash manifest.
Large certificates are separate release assets rather than ordinary
Git objects. The paper must cite a fixed release and commit, rather
than a changing default branch. At the date of this draft the public
repository URL, release identifier, author metadata, and arXiv
submission are not yet assigned; this paragraph must be completed
before submission. Local incremental compilation is recorded separately
from a fresh build of the public release.

\paragraph{Preparation disclosure.}
AI assistance was used in drafting, coding, and auditing this development.
The named human author must review the mathematical claims, references,
and artifacts and assumes responsibility for the submitted work.
This draft has not been submitted or peer reviewed.
