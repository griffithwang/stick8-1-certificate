...
of the finite formula. All 2,294 constraints at the source level participate
...
\section{The finite exclusion}\label{sec:formal}
...
1,577,701 actions. The fixed certificate and its input are available in
the accompanying release; file identifiers and reproduction details
are collected in Appendix~\ref{app:reproduction}.
...
\section{The upper bound and proof of the main theorem}\label{sec:upper}
...
trace, not on equality of a polynomial. The upper-bound certificate is
independent of the SAT refutation and remains outside Lean. The
two-bridge theorem \cite{hno} supplies an independent upper bound.
\par\end{samepage}
...
The formal development uses Lean 4.19.0 and a pinned version of mathlib;
the exact versions are recorded in Appendix~\ref{app:reproduction}.
...
The formal development establishes the geometric implications for 2,283
of the 2,294 source constraints. The global coverage argument, the nine
ear conditions, and the two diagram-invariant conditions are supplied by
the ordinary mathematical proofs in Sections~\ref{sec:roof},
\ref{sec:ears}, and~\ref{sec:diagram}. These counts describe the scope
of formalization, not a probability of correctness. The lower bound
depends on every necessary condition and on the global reduction.

\begin{samepage}
Exact rational controls, an independent Alexander-coefficient calculation,
and separate certificate replays provide supplementary checks; they do
not establish universal geometric coverage. The development does not yet
contain an end-to-end theorem \code{stick(K81) = 10}.
\par\end{samepage}

The code, certificates, and verification records are available in the
fixed release cited in Appendix~\ref{app:reproduction}. They permit
reproduction of the finite checks separately from review of the
...
\appendix
\section{Computational data and reproduction}\label{app:reproduction}
The source repository is
\begin{center}\small
\url{https://github.com/griffithwang/stick8-1-certificate}.
\end{center}
The verified release \code{v0.1.0-audit} is pinned to commit
\begin{verbatim}
0b88e7c65ab01fbe281920946d58ae079191ec39
\end{verbatim}
It contains the source dependency closure, finite data, and a hash
manifest. The large certificates are distributed as release assets.
The file sizes below refer to the exact distributed bytes.
...
File & Bytes & Role\\
\midrule
\code{roof_direct.cnf} & 1,464,784 & Fixed CNF instance\\
\code{roof_base_dense.lrat} & 700,460,899 & Dense textual LRAT proof\\
\code{roof.lz4.hex} & 408,064,310 & Compressed proof for Lean\\
\bottomrule
\end{tabular}
\end{center}
The last file is installed in \code{lean_formalization/}.
Their SHA-256 checksums identify the files used in verification; a
matching checksum establishes file consistency, not mathematical validity.
...
roof_base_dense.lrat
97d9c4224e353ea24ac09b5373be1fb217d2ef057577cb6f01b6475b05190a87

lean_formalization/roof.lz4.hex
3185a627790f0c17bdc4773990f17d55716d003b6ef8ca903c50f0031b75d3de
\end{verbatim}
\endgroup

\paragraph{Software versions and checks.}
Lean is pinned to version 4.19.0 and mathlib to commit
\begin{verbatim}
c44e0c8ee63ca166450922a373c7409c5d26b00b
\end{verbatim}
All 59 local Lean modules were rebuilt successfully on Windows using
the pinned upstream mathlib cache. Per-module source hashes, exit codes,
and timings are recorded in \code{LEAN_RELEASE_VERIFICATION.json}.
The independent arithmetic and C-checker replays have separate reports;
no claim is made that every platform was tested.

After installing the pinned dependencies and obtaining the release assets
as described in the repository README, run from the repository root:
\begin{verbatim}
python verify_public.py
python verify_partial_formalization.py --full
\end{verbatim}
The first command checks the file manifest, replays the exact upper-bound
trace, and reconstructs and checks the base encoding. The second recompiles
all local Lean modules. Omitting \code{--full} performs only the incremental
final-theorem audit. The arithmetic checkers use assertions, so Python
must be run without its optimization flag.

An independent LRAT replay on a system with a C compiler is:
\begin{verbatim}
