...
evens=[sum(p[i]>p[j] for i in range(3) for j in range(i+1,3))%2==0 for p in ps]
lines=['import RoofFrame','', 'namespace Stick81.LineDefinitions',
       'open Geometry RoofFrame GeometricInput SignRules Expression','',
       'def permTable : List (List (Fin 3)) := ['+',\n'.join('['+','.join(map(str,p))+']' for p in ps)+']',
       ...
       '  fin_cases p']
for p,ev in zip(ps,evens):
    lines.append(f'  · change det3 (v {p[0]}) (v {p[1]}) (v {p[2]}) = '+('' if ev else '-')+'det3 (v 0) (v 1) (v 2)')
    lines += ['    rfl'] if p==(0,1,2) else ['    simp only [det3,dot,cross]','    ring']
lines += ['',
...
(ROOT/'LineDefinitions.lean').write_text('\n'.join(lines),encoding='utf-8')
print('Generated line permutation and incidence definitions')
